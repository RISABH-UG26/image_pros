...
typedef struct {
    ...
    XImage_pros_uio_map maps[ MAX_UIO_MAPS ];
} XImage_pros_uio_info;

/***************** Variable Definitions **************************************/
static XImage_pros_uio_info uio_info;
...
static int uio_info_read_name(XImage_pros_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XImage_pros_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XImage_pros_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XImage_pros_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XImage_pros_Initialize(XImage_pros *InstancePtr, const char* InstanceName) {
	XImage_pros_uio_info *InfoPtr = &uio_info;
